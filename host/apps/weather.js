// Weather slide — the second ambient-display feature. Renders current weather onto the 96x160
// portrait picture page: a condition icon, a big current temperature, condition text, today's
// hi/lo, and a location label. Pure render — no network here. The runner's Open-Meteo poll owns
// state; this only draws it. Built to mirror apps/nowplaying.js (themed layout, an accent color
// keyed to the content, a small app-wrapper).
//
// State shape (from lib/weather.js getWeather / getWeatherMock):
...
//
// FONT NOTE: the 5x7 font (lib/font.js) has NO degree glyph — it carries only 0-9, ':', space,
...
import { GLYPH_H, textWidth } from '../lib/font.js';
...
// Accent per condition icon — seeds both the icon body and the bottom bar (same idea as
// now-playing theming its progress bar off the cover art). Deliberately readable on the dark UI.
const ICON_ACCENT = {
...
function iconMoon(fb, cx, cy, accent) {
  ...
  filledCircle(fb, cx + 9, cy - 4, 13, [10, 11, 16]); // bg — must match render()'s bg
}
 ...
 * has only '.'). Same trick nowplaying.js uses so long condition text never spills.
 */
...
export function render(state = {}) {
  const bg = [10, 11, 16];      // must match iconMoon's carve color
...
/**
 * App wrapper for a scheduler. `getState` is a live getter the runner updates from its Open-Meteo
 * poll. fps is effectively "as needed" — weather has no animated element, so 1 is plenty and the
 * runner only re-pushes when the reading changes. Mirrors nowplaying.makeNowPlayingApp.
 * @param {() => object} getState
 */
export function makeWeatherApp(getState) {
  return { id: 'weather', fps: 1, render: () => render(getState() || {}) };
}
